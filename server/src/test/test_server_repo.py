...
from db.server_models import Base, Client, ClientContact
...
class TestRepo:
    ...
    def test_get_client_by_username(self):
        ...
        :return: Client
        """
        leo = self.storage.get_client_by_username('Leo')
        assert leo.Name == 'Leo'
        # что будет если клиента нет
        n = self.storage.get_client_by_username('None')
        # вернется None
        assert n is None

    def test_client_exist(self):
        # такой есть
        assert self.storage.client_exists('Leo')
        # такого нету
        assert not self.storage.client_exists('None')

    def test_add_client(self):
        self.storage.add_client('New')
        assert self.storage.client_exists('New')

    def test_get_contacts(self):
        # возьмем контакты Kate
        contacts = self.storage.get_contacts('Kate')
        assert len(contacts) == 0
        # контакты Leo
        contacts = self.storage.get_contacts('Leo')
        assert len(contacts) == 1
        assert contacts[0].Name == 'Kate'
        # контакты Max
        contacts = self.storage.get_contacts('Max')
        assert len(contacts) == 2
        # контакты неизвестного человека
        contacts = self.storage.get_contacts('None')
        assert [] == contacts
    ...
    def test_add_del_contact(self):
        ...
        self.storage.add_client('New')
        # добавим ему контакт
        self.storage.add_contact('New', 'Kate')
        # проверим
        contacts = self.storage.get_contacts('New')
        assert len(contacts) == 1
        assert contacts[0].Name == 'Kate'
        ...
        with raises(ContactDoesNotExist):
            self.storage.add_contact('New', 'None')
        # удалим контакт которого у него нет
        with raises(ContactDoesNotExist):
            self.storage.del_contact('New', 'None')
